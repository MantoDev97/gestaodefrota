...
const STATE_FILE = path.join(DATA_DIR, "gmail-cassino-state.json");
...
const GMAIL_AUTH_URL_FILE = path.join(DATA_DIR, "gmail-cassino-auth-url.txt");
const GMAIL_DEBUG_FILE = path.join(DATA_DIR, "gmail-cassino-debug.log");
...
main().catch(async (error) => {
  console.error(error);
  await safeNotifyWhatsapp(`Automação Gmail cassino: erro - ${error.message}`);
  process.exitCode = 1;
});
...
async function main() {
  ...
  const state = readJson(STATE_FILE, { trashed: {} });
  const gmail = await createGmailClient();
  await safeNotifyWhatsapp("Automação Gmail cassino: iniciando limpeza.");

  const messageMap = new Map();
  for (const term of SEARCH_TERMS) {
    const query = `in:anywhere -in:trash ${term}`;
    const messages = await listAllMessages(gmail, query);
    for (const message of messages) {
      messageMap.set(message.id, { id: message.id, term });
    }
  }
  ...
  for (const message of messageMap.values()) {
    ...
    try {
      ...
      state.trashed[message.id] = {
        matchedTerm: message.term,
  ...
  await safeNotifyWhatsapp(`Automação Gmail cassino: limpeza concluída. Encontrados: ${messageMap.size}. Movidos para lixeira: ${trashed}. Já processados: ${skipped}. Erros: ${errors.length}.`);
  console.log(JSON.stringify({ ok: errors.length === 0, found: messageMap.size, trashed, skipped, errors }, null, 2));
}
...
async function createGmailClient() {
  ...
  const tokenFile = ensureJsonFileFromEnv("GMAIL_MODIFY_TOKEN_JSON", process.env.GMAIL_CASSINO_OAUTH_TOKEN || GMAIL_TOKEN_FILE);
...
async function createRedirectServer() {
  ...
  const waitForCode = new Promise((resolve, reject) => {
    server = http.createServer((request, response) => {
      ...
      response.end("Gmail autorizado para limpeza. Você pode fechar esta janela e voltar ao Codex.");
      resolve(code);
      server.close();
    });
  });
